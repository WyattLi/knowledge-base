...
export const categories = mysqlTable("categories", {
  ...
  parentId: varchar("parent_id", { length: 36 }).default(sql`NULL`),
...
export const notes = mysqlTable("notes", {
  ...
  contentHash: varchar("content_hash", { length: 64 }).default(sql`NULL`),
  wordCount: int("word_count").default(0),
  status: mysqlEnum("status", ["draft", "published", "archived"]).default("published"),
  aiCategorySuggestion: varchar("ai_category_suggestion", { length: 100 }).default(sql`NULL`),
  aiTagsJson: json("ai_tags_json").default(sql`NULL`),
  aiAnalyzedAt: datetime("ai_analyzed_at").default(sql`NULL`),
  createdAt: datetime("created_at").default(sql`CURRENT_TIMESTAMP`),
  updatedAt: datetime("updated_at").default(sql`CURRENT_TIMESTAMP`),
}, (table) => [
